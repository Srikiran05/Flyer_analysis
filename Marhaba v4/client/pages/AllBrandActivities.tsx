...
const currentCurrency = useMemo(() => {
  ...
  const distinctCountries = new Set(safeProducts.map(p => p.country));
  if (distinctCountries.size > 1) {
    console.warn("Warning: Aggregating data across multiple countries/currencies!");
  }
  return currencyMap[safeProducts[0].country || ''] || '$';
}, [safeProducts]);
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-auto lg:h-[450px]">
        ...
        <div className="lg:col-span-5 bg-zinc-900 border border-zinc-800 rounded-lg p-4 h-[450px] flex flex-col">
          ...
          {isLoading ? (
            <div className="absolute inset-0 flex items-center justify-center bg-zinc-900/80 z-10 h-[300px]">
          ...
          ) : (<div className="flex" style={{ height: `${getDynamicHeight(activeStackData.length)}px` }}>
              {/* Pinned brand-name column: stays put while the bars scroll
                  horizontally. Row heights mirror recharts' even band
                  distribution (same total height, same 5px v-margins). */}
              <div className="w-[120px] shrink-0 flex flex-col py-[5px]">
              ...
              {/* Horizontally scrollable bars: width grows with the number
                  of stack segments so slices stay readable. */}
              <div className="flex-1 min-w-0 overflow-x-auto custom-scrollbar">
              <div style={{ minWidth: '100%', width: `${Math.max(activeStackKeys.length * 56, 560)}px`, height: '100%' }}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart
                  layout="vertical"

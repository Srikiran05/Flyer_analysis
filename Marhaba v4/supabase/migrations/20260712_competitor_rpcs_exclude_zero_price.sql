-- ============================================================
-- COMPETITOR PRICING: ignore offers with null/zero price
--
-- Offers whose offer price is NULL or 0 previously still counted in
-- offer_count (averages already skipped them), inflating counts with
-- rows that carry no price signal. Add a hard row filter to all three
-- RPCs feeding the Competitor Pricing Analysis tab.
-- Grants: authenticated + service_role only (anon stays revoked).
-- ============================================================
...
BEGIN
  IF p_distinct_offers THEN
    ...
    SELECT
      ...
      COUNT(*)::BIGINT AS offer_count,
      ROUND(COALESCE(MIN(CASE WHEN dp.offer_price > 0 THEN dp.offer_price ELSE NULL END), 0::NUMERIC), 2) AS min_price,
      ROUND(COALESCE(MAX(CASE WHEN dp.offer_price > 0 THEN dp.offer_price ELSE NULL END), 0::NUMERIC), 2) AS max_price
    FROM distinct_products dp
  ...
  ELSE
    ...
    SELECT
      ...
      COUNT(*)::BIGINT AS offer_count,
      ROUND(COALESCE(MIN(CASE WHEN fp.offer_price_num > 0 THEN fp.offer_price_num::NUMERIC ELSE NULL END), 0::NUMERIC), 2) AS min_price,
      ROUND(COALESCE(MAX(CASE WHEN fp.offer_price_num > 0 THEN fp.offer_price_num::NUMERIC ELSE NULL END), 0::NUMERIC), 2) AS max_price
    FROM flyer_products fp
    WHERE lower(fp.country) = lower(p_country)
      AND fp.start_date >= cutoff_date
      AND fp.offer_price_num IS NOT NULL AND fp.offer_price_num::NUMERIC > 0
    ...
    WITH distinct_products AS (
      ...
      WHERE fp.brand = target_brand
        ...
        AND (array_length(v_categories, 1) IS NULL OR array_length(v_categories, 1) = 0
             OR fp.category = ANY(v_categories))
...
-- Permissions: authenticated only (anon/PUBLIC stay revoked)
REVOKE EXECUTE ON FUNCTION get_competitor_pricing_aggregations(text,text,text,text,text,text[],text,boolean,integer) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION get_competitor_pack_aggregations(text,text,text,text,text,text[],text,boolean,integer) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION get_retailer_activity(text,text,text[],text[],text[],text[],boolean) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION get_competitor_pricing_aggregations(text,text,text,text,text,text[],text,boolean,integer) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION get_competitor_pack_aggregations(text,text,text,text,text,text[],text,boolean,integer) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION get_retailer_activity(text,text,text[],text[],text[],text[],boolean) TO authenticated, service_role;
